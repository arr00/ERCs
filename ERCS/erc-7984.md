...
Compliant tokens MUST implement [ERC-165](./eip-165.md). The `supportsInterface` function MUST return `true` if `0xa1fb2621` is passed through the `interfaceID` argument.
...
Implementations MAY expose contract-level metadata by additionally implementing [ERC-7572](./eip-7572.md), which defines a `contractURI()` function.
